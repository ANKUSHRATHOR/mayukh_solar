  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Leads This Month" value={String(s.totalLeadsThisMonth)} icon={Users} change={s.totalLeadsThisMonth > 0 ? `${s.totalLeadsThisMonth} new` : 'No leads yet'} changeType={s.totalLeadsThisMonth > 0 ? 'up' : 'neutral'} />
        <StatCard title="Running Projects" value={String(s.runningProjects)} icon={Briefcase} change={s.runningProjects > 0 ? 'Active' : 'None'} changeType={s.runningProjects > 0 ? 'up' : 'neutral'} />
        <StatCard title="Completed This Month" value={String(s.completedThisMonth)} icon={CheckCircle2} change={s.completedThisMonth > 0 ? 'This month' : 'None yet'} changeType={s.completedThisMonth > 0 ? 'up' : 'neutral'} />
        <StatCard title="Cancelled Leads" value={String(s.cancelledLeads)} icon={XCircle} change={s.cancelledLeads > 0 ? `${s.cancelledLeads} cancelled` : 'All clear'} changeType={s.cancelledLeads > 0 ? 'down' : 'neutral'} />
        <StatCard title="Pending Documents" value={String(s.pendingDocuments)} icon={FileText} change="Awaiting upload" changeType={s.pendingDocuments > 0 ? 'down' : 'neutral'} />
        <StatCard title="Revenue This Month" value={`₹${s.revenueThisMonth > 0 ? (s.revenueThisMonth / 100000).toFixed(1) + 'L' : '0'}`} icon={IndianRupee} change={s.revenueThisMonth > 0 ? 'This month' : 'No revenue yet'} changeType={s.revenueThisMonth > 0 ? 'up' : 'neutral'} />
        <StatCard title="Overdue Follow-ups" value={String(s.overdueFollowUps)} icon={Clock} change={s.overdueFollowUps > 0 ? 'Needs attention' : 'All clear'} changeType={s.overdueFollowUps > 0 ? 'down' : 'neutral'} />
        <StatCard title="Pending Loan Files" value={String(s.pendingLoanFiles)} icon={FolderOpen} change={s.pendingLoanFiles > 0 ? 'In process' : 'None'} changeType={s.pendingLoanFiles > 0 ? 'down' : 'neutral'} />
      </div>
