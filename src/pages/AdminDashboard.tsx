  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard onClick={() => navigate('/admin/projects?status=project_completed')} title="Completed This Month" value={String(s.completedThisMonth)} icon={CheckCircle2} change={s.completedThisMonth > 0 ? 'This month' : 'None yet'} changeType={s.completedThisMonth > 0 ? 'up' : 'neutral'} />
        ...
        <StatCard onClick={() => navigate('/admin/projects?status=pending_documents')} title="Pending Documents" value={String(s.pendingDocuments)} icon={FileText} change="Awaiting upload" changeType={s.pendingDocuments > 0 ? 'down' : 'neutral'} />
        <StatCard onClick={() => navigate('/admin/projects?status=project_completed')} title="Revenue This Month" value={`₹${s.revenueThisMonth > 0 ? (s.revenueThisMonth / 100000).toFixed(1) + 'L' : '0'}`} icon={IndianRupee} change={s.revenueThisMonth > 0 ? 'This month' : 'No revenue yet'} changeType={s.revenueThisMonth > 0 ? 'up' : 'neutral'} />
        <StatCard onClick={() => navigate('/leads?status=follow_up')} title="Overdue Follow-ups" value={String(s.overdueFollowUps)} icon={Clock} change={s.overdueFollowUps > 0 ? 'Needs attention' : 'All clear'} changeType={s.overdueFollowUps > 0 ? 'down' : 'neutral'} />
        <StatCard onClick={() => navigate('/admin/projects?status=loan_process')} title="Pending Loan Files" value={String(s.pendingLoanFiles)} icon={FolderOpen} change={s.pendingLoanFiles > 0 ? 'In process' : 'None'} changeType={s.pendingLoanFiles > 0 ? 'down' : 'neutral'} />
      </div>
