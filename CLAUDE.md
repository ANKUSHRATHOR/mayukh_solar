...
```
PageContainer → PageHeader → StatStrip (optional) → Tabs → TableToolbar → DataTable → TablePagination
```
...
Use `layout` `auto` (the default: table from `md`, cards below) unless there is a reason not to. Forcing `cards` also forces the SORT select, because there are no headers to click.
